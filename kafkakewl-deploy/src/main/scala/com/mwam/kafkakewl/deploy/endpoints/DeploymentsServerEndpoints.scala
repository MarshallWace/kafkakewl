...
class DeploymentsServerEndpoints(
  ...
  private def getDeployment(
  ...
  ): ZIO[Any, QueryDeploymentsFailure, TopologyDeployment] = for {
    _ <- tracing.addEvent("reading topology from cache")
    _ <- tracing.setAttribute("topology_id", topologyId.value)
    topologyDeployment <- topologyDeploymentsService.getTopologyDeployment(
      topologyId
    )
    ...
    topologyDeployment <- ZIO.getOrFailWith(
      DeploymentsFailure.notFound(s"topology $topologyId not found")
    )(topologyDeployment)
  } yield topologyDeployment

  private def getDeployments(
      deploymentQuery: TopologyDeploymentQuery
  ): ZIO[Any, QueryDeploymentsFailure, Seq[TopologyDeployment]] =
    topologyDeploymentsService.getTopologyDeployments(deploymentQuery)

  private def postDeployments(
      deployments: Deployments
  ): ZIO[Any, PostDeploymentsFailure, DeploymentsSuccess] =
    topologyDeploymentsService.deploy(deployments)
}

object DeploymentsServerEndpoints {
  val live: ZLayer[
    DeploymentsEndpoints & TopologyDeploymentsService & Tracing,
    Nothing,
    DeploymentsServerEndpoints
  ] =
