...
object DeploymentsFailure {
  ...
  def deployment(throwable: Throwable): Deployment = Deployment(
    errors(throwable)
  )
  def persistence(throwable: Throwable): Persistence = Persistence(
    errors(throwable)
  )

  private def errors(throwable: Throwable): Seq[String] = Seq(
...
/** The result of a successful deployment.
  ...
  */
final case class DeploymentsSuccess(
    statuses: Map[TopologyId, TopologyDeploymentStatus]
)
