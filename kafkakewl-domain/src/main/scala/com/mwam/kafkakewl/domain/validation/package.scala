...
package object validation {
  ...
  def invalid(validationError: ValidationError): ValidationFailures =
    Validation.fail(validationError)

  def validationErrorIf(error: => ValidationError)(
      predicate: => Boolean
  ): ValidationFailures =
  ...
  extension (validationFailures: ValidationFailures) {
    @targetName("add")
    def +(other: ValidationFailures): ValidationFailures =
      combine(validationFailures, other)
  }

  extension (validationFailures: Iterable[ValidationFailures]) {
    def combine: ValidationFailures =
      validationFailures.reduceOption(_ + _).getOrElse(valid)
  }
}
