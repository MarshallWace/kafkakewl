...
class TopicServerEndpoints(
    topicEndpoints: TopicEndpoints,
    topicService: KafkaTopicInfoCache,
    tracing: Tracing
) {
  ...
  val endpoints: List[ZServerEndpoint[Any, Any]] = List(
    ...
    topicEndpoints.getTopicEndpoint.zServerLogicWithTracing(topic =>
      getTopic(topic)
    )
  )

  private def getTopics: ZIO[Any, QueryFailure, Seq[String]] =
    topicService.getTopics

  private def getTopic(
      topic: String
  ): ZIO[Any, QueryFailure, KafkaSingleTopicPartitionInfos] = for {
    ...
    withErrorType <- ZIO.getOrFailWith(
      Failures.notFound(s"topic $topic not found")
    )(topicPartitionInfos)
  } yield withErrorType
}
...
object TopicServerEndpoints {
  val live: ZLayer[
    TopicEndpoints & KafkaTopicInfoCache & Tracing,
    Nothing,
    TopicServerEndpoints
  ] =
