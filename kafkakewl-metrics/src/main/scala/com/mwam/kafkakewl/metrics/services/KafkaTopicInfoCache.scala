...
class KafkaTopicInfoCache(
    private val kafkaTopicInfosRef: Ref[
      Map[String, KafkaSingleTopicPartitionInfos]
    ]
) {
  def getTopics: UIO[Seq[String]] =
    kafkaTopicInfosRef.get.map(_.keys.toSeq.sorted)
  def getTopicPartitionInfos(
      topic: String
  ): UIO[Option[KafkaSingleTopicPartitionInfos]] =
    kafkaTopicInfosRef.get.map(_.get(topic))
}
...
object KafkaTopicInfoCache {
  def live: ZLayer[KafkaTopicInfoSource, Nothing, KafkaTopicInfoCache] =
    ZLayer.scoped {
      for {
        topicChangesDequeue <- ZIO.serviceWithZIO[KafkaTopicInfoSource](
          _.subscribe()
        )
        kafkaTopicInfosRef <- Ref.make(
          Map.empty[String, KafkaSingleTopicPartitionInfos]
        )
        processTopicChangesFiber <- processTopicChanges(
          topicChangesDequeue,
          kafkaTopicInfosRef
        ).forever.fork
