...
class KafkaTopicInfoSource(
    ...
    private val topicInfoChangesStream: ZStream[
      Any,
      Throwable,
      KafkaTopicPartitionInfoChanges
    ],
  ...
  def subscribe(): URIO[Scope, Dequeue[KafkaTopicPartitionInfoChanges]] =
    hub.subscribe
  ...
  def live: ZLayer[Consumer, Nothing, KafkaTopicInfoSource] =
    ZLayer.scoped {
      for {
        ...
        hub <- Hub.bounded[KafkaTopicPartitionInfoChanges](requestedCapacity =
          1
        )
      } yield KafkaTopicInfoSource(
        scope,
        createKafkaTopicInfosStream(consumer),
        hub
      )
    }

  private def createKafkaTopicInfosStream(
      consumer: Consumer,
      timeout: Duration = 10.seconds
  ): ZStream[Any, Throwable, KafkaTopicPartitionInfoChanges] = {
    val queryTopicPartitionInfos = (for {
      ...
      topicPartitions = kafkaTopicPartitionInfos
        ...
        .filter { case (topic, _) =>
          topic.startsWith("kewltest") || topic.startsWith(
            "streambook"
          ) || topic.startsWith("dbz")
        }
        .flatMap { case (topic, kafkaPartitionInfos) =>
          kafkaPartitionInfos.map(tpi =>
            new TopicPartition(topic, tpi.partition)
          )
        }
        .toSet
      beginningEndOffsets <- consumer.beginningOffsets(
        topicPartitions,
        timeout
      ) zip consumer.endOffsets(topicPartitions, timeout)
    } yield {
      ...
      ZIO.logError(
        s"failed to get the topics' beginning and end offsets: ${t.getMessage}"
      )
    ).fold(_ => None, topicPartitionInfos => Some(topicPartitionInfos))

    ZStream
      .repeatZIOWithSchedule(
        queryTopicPartitionInfos.timed,
        Schedule.fixed(5.seconds)
      )
      // removing the none items because they are failed queries
      .collect { case (duration, Some(topicPartitionInfos)) =>
        (duration, topicPartitionInfos)
      }
      .tap { case (duration, topicPartitionInfos) =>
        ZIO.logInfo(
          s"received ${topicPartitionInfos.size} topic-partition beginning-end offsets in ${duration.toMillis / 1000.0} seconds"
        )
      }
      ...
      .filter { case (previous, current) =>
        previous.nonEmpty || current.nonEmpty
      }
      // diffing the current to the previous and emitting the changes
      .map { case (previous, current) => previous.diff(current) }
      .tap { changes =>
        ZIO.logInfo(
          s"emitting ${changes.addedOrUpdated.size} added or updated topic-partition infos and ${changes.removed.size} removed ones"
        )
      }
  }
}
