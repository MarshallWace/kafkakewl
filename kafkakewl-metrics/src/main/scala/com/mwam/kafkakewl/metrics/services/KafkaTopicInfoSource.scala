...
object KafkaTopicInfoSource {
  ...
  private def createKafkaTopicInfosStream(
    ...
    ZStream
      .repeatZIOWithSchedule(queryTopicPartitionInfos.timed, Schedule.fixed(5.seconds))
      ...
      .tap { case (duration, topicPartitionInfos) =>
        ZIO.logInfo(s"received ${topicPartitionInfos.size} topic-partition beginning-end offsets in ${duration.toMillis / 1000.0} seconds")
      }
      ...
      .scan((KafkaTopicPartitionInfos.empty, KafkaTopicPartitionInfos.empty)) { case ((_, previous), current) =>
        (previous, current)
      }
      ...
      .tap { changes =>
        ZIO.logInfo(s"emitting ${changes.addedOrUpdated.size} added or updated topic-partition infos and ${changes.removed.size} removed ones")
      }
  }
}
