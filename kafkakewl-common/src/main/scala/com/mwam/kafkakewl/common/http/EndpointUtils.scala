...
object EndpointUtils {
  ...
  /**
   * Body codec that will deserialize YAML into domain objects via YAML -> JSON -> domain
   * marshalling. This codec does not support encoding responses as YAML (since circe-yaml
   * does not support encoding) and YAML response are not expected to see use.
   *
   * The Content-Type of YAML requests is associated with "text/plain".
   *
   * This works by using circe-yaml to convert the YAML input from the request body into JSON
   * and using the existing tapir JSON marshalling onwards.
   *
   * @tparam T Domain object target of the model
   * @return A body mapping from string to T
   */
