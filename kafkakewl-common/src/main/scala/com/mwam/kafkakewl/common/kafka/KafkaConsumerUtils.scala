  ...
  extension (topicPartitions: List[TopicPartition]) {
    def distribute(parallelism: Int): List[List[TopicPartition]] = {
      topicPartitions.zipWithIndex
        ...
        .map { case (threadIndex, tps) =>
          (threadIndex, tps.map { case (tp, _) => tp })
        }
...
final case class CompactedConsumeResult[Key, Value](
  ...
  @targetName("add")
  def +(
      other: CompactedConsumeResult[Key, Value]
  ): CompactedConsumeResult[Key, Value] = CompactedConsumeResult(
...
object KafkaConsumerUtils {
  ...
  def kafkaConsumerStringStringZIO(
      kafkaClientConfig: KafkaClientConfig
  ): RIO[Scope, Consumer[String, String]] =
    kafkaConsumer(
      kafkaClientConfig,
  ...
  def kafkaConsumerBytesBytesZIO(
      kafkaClientConfig: KafkaClientConfig
  ): RIO[Scope, Consumer[Bytes, Bytes]] =
    kafkaConsumer(
      kafkaClientConfig,
  ...
  def consumeUntilEnd[Key, Value](
    ...
    val nextTopicPartitionOffsets =
      mutable.Map.empty[TopicPartition, Long] ++ consumer.positions(
        topicPartitions
      )
    while (shouldConsume) {
      val consumerRecords = consumer.poll(pollTimeoutJava)
      val nextOffsets = consumer.positions(topicPartitions)
      shouldConsume = endOffsets.exists { case (tp, endOffset) =>
        nextOffsets.get(tp).exists(nextOffset => nextOffset < endOffset)
      }
      handleConsumerRecords(consumerRecords)
      consumerRecords.forEach { cr =>
        nextTopicPartitionOffsets.update(
  ...
  def consumeCompactUntilEnd[Key, Value](
    ...
    CompactedConsumeResult(
      if (removeTombstones) {
        lastValues.view.collect { case (key, Some(value)) =>
          (key, Some(value))
        }.toMap
      } else {
        lastValues.toMap
