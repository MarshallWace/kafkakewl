...
object BatchMessageEnvelopeJson {
  ...
  given [Payload](using
      JsonEncoder[Payload]
  ): JsonEncoder[BatchMessageEnvelope[Payload]] =
    DeriveJsonEncoder.gen[BatchMessageEnvelope[Payload]]
  given [Payload](using
      JsonDecoder[Payload]
  ): JsonDecoder[BatchMessageEnvelope[Payload]] =
    DeriveJsonDecoder.gen[BatchMessageEnvelope[Payload]]
}
...
class KafkaPersistentStore(
  ...
  private object KafkaSerde {
    ...
    val value: Serde[Any, BatchMessageEnvelope[TopologyDeployment]] =
      Serde.string.inmapM[Any, BatchMessageEnvelope[TopologyDeployment]](s =>
        ZIO
          .fromEither(s.fromJson[BatchMessageEnvelope[TopologyDeployment]])
  ...
  override def loadLatest(): Task[TopologyDeployments] =
    initializeIfNeeded *>
      ZIO.scoped {
        for {
          consumer <- KafkaConsumerUtils.kafkaConsumerStringStringZIO(
            kafkaClientConfig
          )
          _ <- ZIO.logInfo(
            s"loading the topologies from the $topicName kafka topic"
          )
          deserializedTopologiesWithDuration <- consumeUntilEnd(
            consumer,
            topicName
          ).timed
  ...
  ): Stream[Throwable, TopologyDeployments] =
  ...
  private def createTopicZIO(adminClient: AdminClient) = {
    ...
    val newTopic =
      AdminClient.NewTopic(topicName, numPartitions, replicationFactor, config)
    for {
  ...
  private def deleteTopicZIO(adminClient: AdminClient) =
    ZIO.logInfo(s"Deleting topic $topicName...") *> adminClient.deleteTopic(
      topicName
    )
  ...
  private def recreateTopicZIO(adminClient: AdminClient) = {
    ...
    deleteTopicZIO(adminClient)
      *> ZIO.logInfo(
        s"Waiting for ${delay.toSeconds} seconds before creating topic $topicName..."
      )
      *> ZIO.sleep(delay)
      *> createTopicZIO(adminClient)
  }
  ...
  private def updateTopicZIO(adminClient: AdminClient) = {
    for {
      ...
      topicDescriptions <- ZIO.getOrFailWith(
        RuntimeException(s"Could not describe topic $topicName")
      )(topicsDescriptions.get(topicName))

      numPartitions <- ZIO.getOrFailWith(
        RuntimeException(s"Topic $topicName has no partition infos.")
      ...
      _ <- ZIO.logInfo(
        s"Topic $topicName partitions = $numPartitions, replicas = $numReplicas, config = $config"
      )
    } yield ()
  }

  private def consumeUntilEnd(
      consumer: Consumer[String, String],
      topic: String
  ): Task[Map[TopologyId, Either[String, TopologyDeployment]]] =
    ZIO.attempt {
      ...
      KafkaConsumerUtils.consumeUntilEnd(consumer, topicPartitions) {
        _.forEach(cr => messages += (cr.key -> cr.value))
      }
      ...
      messages
        ...
        .map { case (key, value) =>
          (
            TopologyId(key),
            value
              .fromJson[BatchMessageEnvelope[TopologyDeployment]]
              .map(_.payload)
          )
        }.toMap
    }
  ...
  private val defaultKafkaTopicConfig = Map(
...
object KafkaPersistentStore {
  val live: ZLayer[
    KafkaClientConfig & KafkaPersistentStoreConfig,
    Throwable,
    ...
    ZLayer.scoped {
      ...
      } yield KafkaPersistentStore(
        kafkaClientConfig,
        kafkaPersistentStoreConfig,
        producer,
        initialized
      )
    }
