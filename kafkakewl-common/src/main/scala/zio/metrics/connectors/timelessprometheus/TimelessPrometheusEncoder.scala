...
case object TimelessPrometheusEncoder {
  def encode(event: MetricEvent): ZIO[Any, Throwable, Chunk[String]] =
    ZIO.attempt(encodeMetric(event.metricKey, event.current))
  ...
  private def encodeMetric(
    ...
    def encodeExtraLabels(extraLabels: Set[MetricLabel]) =
      if (extraLabels.isEmpty) baseLabels
      else encodeLabels(key.tags ++ extraLabels)

    def encodeCounter(
        c: MetricState.Counter,
        extraLabels: MetricLabel*
    ): String =
      s"$name${encodeExtraLabels(extraLabels.toSet)} ${c.count}"

    def encodeGauge(g: MetricState.Gauge): String =
      s"$name$baseLabels ${g.value}"
    ...
    def encodeSamples(samples: SampleResult, suffix: String): Chunk[String] =
      Chunk(
        samples.buckets
          .foldLeft(new StringBuilder(samples.buckets.size * 100)) {
            case (sb, (l, v)) =>
              sb.append(name)
                .append(suffix)
          ...
          .toString,
        s"${name}_sum$baseLabels ${samples.sum}",
        s"${name}_count$baseLabels ${samples.count}",
        s"${name}_min$baseLabels ${samples.min}",
        s"${name}_max$baseLabels ${samples.max}"
      )
    ...
    def sampleSummary(s: MetricState.Summary): SampleResult =
      SampleResult(
        ...
        buckets = s.quantiles.map(q =>
          Set(
            MetricLabel("quantile", q._1.toString),
      ...
      case s: MetricState.Frequency =>
        Chunk.fromIterable(
          s.occurrences
            .map { o =>
              encodeCounter(
                MetricState.Counter(o._2.doubleValue()),
                MetricLabel("bucket", o._1)
              )
            }
        )
    }
